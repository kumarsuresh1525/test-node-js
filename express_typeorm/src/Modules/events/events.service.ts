...
export class EventsService {
  ...
  private eventWorkshopsRepository: Repository<Workshop>;

  constructor(app: App) {
    this.eventRepository = app.getDataSource().getRepository(Event);
    this.eventWorkshopsRepository = app.getDataSource().getRepository(Workshop);
  }
  ...
  async getEventsWithWorkshops() {
    ...
    const eventWorkshops = await this.eventWorkshopsRepository.find();
    const map = new Map();
    eventWorkshops.forEach(item => {
      if (!map.has(item.eventId)) {
        const workshops = [item]
        map.set(item.eventId, workshops);
      } else {
        const workshops = map.get(item.eventId);
        workshops.push(item);
        map.set(item.eventId, workshops);
      }
    });
    const results = events.map((item: any) => {
      item.workshops = map.get(item.id)
      return item;
    })
    return results;
  }
  ...
  async getFutureEventWithWorkshops() {
    ...
    console.log("result++", results.filter((item: any) => !item.workshops))
    return results.filter((item: any) => !item.workshops);
  }
}
